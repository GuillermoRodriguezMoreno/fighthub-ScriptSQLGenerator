...
INSERT INTO fighter (   birth_date, email, password, register_date, user_name, profile_photo_id, active, biography, 
    ...
    VALUES ('{birth_date}', '{email}', '{password}', '{register_date}', '{user_name}', 
            {f"'{profile_photo_id}'" if profile_photo_id else 'NULL'}, {active}, '{biography}', 
            {draws}, '{gender}', {height:.2f}, {kos}, {latitude:.6f}, {longitude:.6f}, {losses}, 
            '{name}', {number_of_fights}, {weight:.2f}, {wins}, {wins_in_a_row}, {category_id});
"""
...
# PROFILE PHOTO OF FIGHTERS
def generate_fighter_profile_photo_insert_statement(total_fighters):
    insert_statement = "-- FIGHTER PROFILE PHOTO"
    URL = "./assets/img/fighter/fighter"
    for fighter_id in range(1, total_fighters + 1):
        entity_type = 'FIGHTER'
        upload_date = fake.date_time_this_year(before_now=True).strftime('%Y-%m-%d %H:%M')
...
"""
        ...
        # #################################### #
        # ASSOCIATE PROFILE PHOTO WITH FIGHTER #
        # #################################### #
        #### UPDATE STATEMENT ####
        insert_statement += f"""
UPDATE fighter SET profile_photo_id = {fighter_id} WHERE id = {fighter_id};
"""
        insert_statement = insert_statement.strip()
        insert_statement += "\n"
...
# PROFILE PHOTO OF CLUBS
def generate_club_photo_insert_statement(total_clubs):
    insert_statement = "-- CLUB PROFILE PHOTO"
    URL = "./assets/img/club/club"
...
"""
        ...
        # ################################# #
        # ASSOCIATE PROFILE PHOTO WITH CLUB #
        # ################################# #
        #### UPDATE STATEMENT ####
        insert_statement += f"""
UPDATE club SET profile_photo_id = {club_id} WHERE id = {club_id};
"""
        insert_statement = insert_statement.strip()
        insert_statement += "\n"
...
def generate_club_membership_request_insert_statement(total_fighters, total_clubs):
    ...
    for fighter_id in range(1, total_fighters + 1):
        ...
        response_date = fake.date_time_between(start_date=request_date, end_date=datetime.now().date()).strftime(
            '%Y-%m-%d %H:%M')
        request_date = request_date.strftime('%Y-%m-%d %H:%M')
        status = random.choice(['PENDING', 'APPROVED', 'REJECTED'])
        club_id = random.randint(1, total_clubs)
...
INSERT INTO club_membership_request (   request_date, response_date, status, club_id, fighter_id)
    VALUES ('{request_date}', '{response_date}', '{status}', {club_id}, {fighter_id});
"""
        insert_statement = insert_statement.strip()
        insert_statement += "\n"
...
INSERT INTO club_review (content, rating, review_date, club_id, fighter_id)
    VALUES ('{content}', {rating}, '{review_date}', {club_id}, {fighter_id});
"""
            insert_statement = insert_statement.strip()
            insert_statement += "\n"
...
INSERT INTO event_review (content, rating, review_date, event_id, fighter_id)  
    VALUES ('{content}', {rating}, '{review_date}', {event_id}, {fighter_id});
"""
            insert_statement = insert_statement.strip()
            insert_statement += "\n"
...
INSERT INTO fight ( fight_order, is_title_fight, minutes, total, start_time, weight, is_ko, 
                    ...
                    winner_id)
    VALUES ({fight_order}, {is_title_fight}, {minutes}, {total}, '{start_time}', {weight:.2f}, {isKo},
            {blue_corner_fighter_id}, {category_id}, {event_id}, {red_corner_fighter_id}, {style_id}, 
        ...
        for event_id in range(1, total_events + 1):
            for fight_id in range(1, 5 + 1):
                ...
                if status == 'APPROVED' or status == 'REJECTED':
                    response_date = fake.date_time_between(start_date=request_date, end_date=datetime.now()).strftime(
                        '%Y-%m-%d %H:%M')
...
INSERT INTO fight_inscription_request ( message, request_date, response, response_date, status, club_id,
    ...
    VALUES ('{message}', '{request_date}', {f"'{response}'" if response else 'NULL'}, 
            {f"'{response_date}'" if response_date else 'NULL'}, '{status}', '{club_id}',
            '{fight_id}', {fighter_id}, {event_id});
"""
                insert_statement = insert_statement.strip()
                insert_statement += "\n"
